...
NAME="UnblockNeteaseMusic1"
NAME_PID="UnblockNeteaseMusic1"
NAME_SERVICE="unblock1631"
FOLDER="/usr/local/UnblockNeteaseMusic1"
FILE="${FOLDER}/app.js"
FOLDER_NODE="/usr/local/UnblockNeteaseMusic1/node"
FILE_NODE="${FOLDER_NODE}/bin/node"
FILE_CONF="${FOLDER}/UnblockNeteaseMusic1.conf"
FILE_LOG="${FOLDER}/UnblockNeteaseMusic1.log"
...
# 更新脚本
_UPDATE_SHELL(){
	NEW_VER_SHELL=$(wget --no-check-certificate -qO- -t1 -T3 "http://www.baidu.com"|grep 'NOW_VER_SHELL="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z "${NEW_VER_SHELL}" ]] && echo -e "${ERROR} 获取脚本最新版本失败！无法链接到 Github !" && exit 1
		...
		wget -N --no-check-certificate "http://www.baidu.com"
		chmod +x "${FILEPASH_NOW}/unblock1631.sh"
...
[[ "${SYSTEM_BIT}" !=  "x86_64" ]] && echo -e "${ERROR} ${NAME} 的依赖 Node 不支持当前系统位数 ${SYSTEM_BIT} !" && exit 1
	...
	echo && echo -e "  UnblockNeteaseMusic1 一键脚本 ${RED_FONT_PREFIX}[v${NOW_VER_SHELL}]${FONT_COLOR_SUFFIX}
